...
def _setup_logging() -> Path:
    ...
    # Ruis van derde-partij libraries (netwerkverkeer bij modeldownloads e.d.)
    # onderdrukken — onze eigen DEBUG-diagnostiek (bv. "raw track"-regels en
    # diarisatie-hyperparameters) blijft wel gewoon gelogd.
    for noisy in ("httpx", "httpcore", "urllib3", "fsspec", "filelock"):
    ...
    try:
        ...
        class _AudioDecoder:
            ...
            def get_samples_played_in_range(self, start, end):
                ...
                # Afronden i.p.v. afkappen (int() gooit tot 1 sample per
                # crop weg) en altijd minstens 1 sample teruggeven: bij
                # zeer korte segmenten (korte testclips, snelle sprekers-
                # wissels) kon int(start*sr) == int(end*sr) een LEEG
                # fragment opleveren. Pyannote's embedding-model rekent
                # dan std/mean over een lege array -> NaN -> die spreker
                # wordt bij het clusteren ten onrechte "gelijk" aan een
                # andere behandeld (mogelijke oorzaak van foutief
                # samengevoegde sprekers — wordt hieronder gelogd zodat
                # dit met een echte testrun te verifiëren is).
                start_frame = int(round(start * sr))
                stop_frame  = int(round(end * sr))
                if stop_frame <= start_frame:
                    stop_frame = start_frame + 1
                start_frame = max(0, min(start_frame, total_frames))
                stop_frame  = max(start_frame + 1, min(stop_frame, total_frames))
                if start_frame >= total_frames:
                    data = _np.zeros((1, self.metadata.num_channels), dtype="float32")
                    log.debug(
                        "audio-fallback crop BUITEN BEREIK: start=%.4fs end=%.4fs "
                        "(frame %d >= totaal %d) -> stilte teruggegeven",
                        start, end, start_frame, total_frames,
                    )
                else:
                    data, _ = _sf.read(
                        self._path,
                    ...
                    if data.shape[0] == 0:
                        data = _np.zeros((1, self.metadata.num_channels), dtype="float32")
                        log.debug(
                            "audio-fallback crop LEEG: start=%.4fs end=%.4fs "
                            "frames[%d:%d] -> stilte teruggegeven",
                            start, end, start_frame, stop_frame,
                        )
                return _AudioSamples(_torch.from_numpy(data.T), sr)
        ...
        # LET OP: NIET meer "return" bij een geslaagde bare import van
        # torchcodec.decoders.AudioDecoder. Een geslaagde Python-import
        # bewijst alleen dat het pakket importeerbaar is — de echte
        # native library (libtorchcodec_coreN.dll) wordt pas LAZY geladen
        # zodra AudioDecoder(...) daadwerkelijk aangeroepen wordt (diep in
        # pyannote's inference-pad). Op sommige Windows-machines (missende
        # FFmpeg shared libs / PyTorch-versie mismatch) faalt die native
        # load alsnog met "Could not load libtorchcodec", terwijl de bare
        # import hierboven allang geslaagd was. Vroeger werd dat hier ten
        # onrechte als "torchcodec beschikbaar" bestempeld en sloeg de
        # patch zichzelf over — waardoor diarisatie alsnog crashte en
        # stilzwijgend terugviel op "Spreker ?". De soundfile-fallback
        # hieronder is onafhankelijk van torchcodec en wordt daarom nu
        # ALTIJD toegepast, ongeacht of torchcodec importeerbaar lijkt.
        ...
        # KRITIEK: io.py checkt vooraf een module-level boolean
        # (TORCHCODEC_AVAILABLE, gezet bij import van pyannote.audio.core.io)
        # en gooit al een RuntimeError vóórdat AudioDecoder() ooit aangeroepen
        # wordt als deze False is. Zonder deze regel heeft het patchen van
        # AudioDecoder/AudioStreamMetadata hierboven dus geen enkel effect —
        # de fallback-klassen worden nooit bereikt.
        _io_mod.TORCHCODEC_AVAILABLE = True
        ...
        # Forceer overschrijven (niet setdefault): als torchcodec al eerder
        # succesvol geïmporteerd is door pyannote.audio.core.io zelf (de
        # bare "import torchcodec" bovenin dat bestand slaagt namelijk
        # los van of de native library werkt), staat de ECHTE module al
        # in sys.modules. setdefault zou dan niets doen en een eventuele
        # latere "from torchcodec.decoders import AudioDecoder" ergens
        # anders in de pyannote-codebase zou alsnog de kapotte, echte
        # decoder teruggeven.
        sys.modules["torchcodec"]          = _fake_tc
        sys.modules["torchcodec.decoders"] = _fake_dec
...
class OutputFormatter:
    ...
    def _speaker(segments: list[dict], language: str) -> str:
        ...
        for s in segments:
            ...
            lines.append(f"{speaker} [{ts}]: {s.get('text', '').strip()}")
        return "\n".join(lines)
...
SPEAKER_TIPS = """Tips voor een optimaal resultaat

Model
"Globaal-snel" is prima voor een snelle ruwe versie. Kies "Nauwkeurig-langzaam" \
als namen, vaktermen of een kloppende sprekerscheiding belangrijk zijn.

Max sprekers
Zet dit zo dicht mogelijk bij het werkelijke aantal deelnemers. Boven de \
±10 sprekers wordt onderscheid tussen stemmen onbetrouwbaar.

Voor betrouwbare sprekerherkenning
- Laat sprekers niet door elkaar heen praten — overlappende spraak is de \
belangrijkste oorzaak van foute sprekerlabels.
- Een korte stilte tussen twee sprekers (ruwweg een halve seconde) helpt \
de herkenning aanzienlijk.
- Zeer korte opnames (een paar seconden per spreker) leveren onbetrouwbare \
sprekerscheiding op — er is wat aaneengesloten spraak per spreker nodig.

Bekende beperking
Korte bevestigingen zoals "ja", "uhm" of "oké" tussen twee zinnen van een \
andere spreker worden soms gemist of aan de verkeerde spreker toegeschreven. \
Dit ligt aan het onderliggende spraakherkenningsmodel en is niet met \
instellingen op te lossen — gebruik "Bewerk transcript" om dit handmatig \
recht te zetten.

Na de transcriptie
Gebruik "Hernoem sprekers" om Spreker A/B te vervangen door echte namen, \
en "Bewerk transcript" om kleine fouten te corrigeren voordat je opslaat."""
...
class TranscribeWorker:
    ...
    def _pipeline(self) -> None:
        ...
        # Vertaal "large" naar echte modelnaam (large-v3)
        model_name = "large-v3" if self.model == "large" else self.model
    ...
    def _diarize(self, segments: list[dict], wav_path: str) -> list[dict]:
        ...
        try:
            log.info("Diarisatie hyperparameters (geinstantieerd): %s",
                      pipeline.parameters(instantiated=True))
        except Exception as _e:
            log.info("Kon diarisatie hyperparameters niet ophalen: %s", _e)
        ...
        for _t, _sp in tracks:
            log.debug("  raw track: %.3f-%.3fs (%.3fs) -> %s",
                      _t.start, _t.end, _t.end - _t.start, _sp)
        ...
        # ── Woord-niveau matching (Sprekerlabels met word_timestamps) ──
...
def _build_gui(controller: "AppController"):
    ...
    saved_lang = controller.config.get("language")
    labeled_combo(card_settings, "Taal:", controller._lang_var,
                  ["nl — Nederlands", "en — Engels", "auto — Automatisch"],
                  LANG_LABELS.get(saved_lang, "nl — Nederlands"))

    saved_model = controller.config.get("model")
    labeled_combo(card_settings, "Model:", controller._model_var,
                  list(MODEL_LABELS.values()),
                  MODEL_LABELS.get(saved_model, "Nauwkeurig-langzaam"))

    labeled_combo(card_settings, "Uitvoermodus:", controller._mode_var,
                  list(MODE_LABELS.values()),
                  MODE_LABELS["3"])

    # Diarisatie-model is niet meer instelbaar in de GUI — altijd de
    # nauwkeurige pipeline (gaf in tests geen ander resultaat dan de
    # "snellere" variant, dus die keuze voegde niets toe).
    controller._diarize_model_var.set("Nauwkeurig (aanbevolen)")
    ...
    tk.Label(spk_row, text="(auto of 1-10)", bg=C_PANEL, fg=C_MUTED,
             font=("Segoe UI", 8)).pack(side=tk.LEFT, padx=(6, 0))

    # HF-token
    tk.Label(card_settings, text="HF-token (Sprekerlabels):", bg=C_PANEL, fg=C_TEXT,
    ...
    def _on_output_browse():
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("Tekstbestand", "*.txt"), ("Alle", "*.*")])
        if path:
    ...
    tips_label = tk.Label(card_actions, text="ⓘ Tips voor beste resultaat",
                          bg=C_PANEL, fg=C_MUTED, font=("Segoe UI", 8, "underline"),
                          cursor="hand2", anchor="w")
    tips_label.pack(fill=tk.X, pady=(0, 4))
    tips_label.bind("<Button-1>", lambda _e: controller.show_speaker_tips())
...
class AppController:
    ...
    def start_transcription(self) -> None:
        ...
        model_label   = (self._model_var.get() or "Nauwkeurig-langzaam")
        model         = next((k for k, v in MODEL_LABELS.items()
                              if v == model_label), "large")
        mode_label    = (self._mode_var.get() or MODE_LABELS["3"])
        mode          = next((k for k, v in MODE_LABELS.items()
                              if v == mode_label), "3")
    ...
    def show_speaker_tips(self) -> None:
        import tkinter.messagebox as mb
        mb.showinfo("Tips voor beste resultaat", SPEAKER_TIPS, parent=self.root)
    ...
    def rename_speakers(self) -> None:
        ...
        if not speakers:
            self._show_error(
                ...
                "Gebruik uitvoermodus 'Sprekerlabels' voor sprekerherkenning."
            )
            return
...
class SkeletonTests:
    ...
    def _t5_worker_thread(self) -> None:
        try:
            ...
            results = []
            TranscribeWorker(
                "__nonexistent__.mp3", "nl", "medium", "3", "", None,
...
def main() -> None:
    ...
    if pyi_splash:
        # Zodra Python draait is het onefile-uitpakken klaar; vervang de
        # bestandsnamen die de bootloader tijdens het uitpakken toonde
        # door een vriendelijke boodschap voor de resterende laadtijd.
        pyi_splash.update_text("Applicatie wordt gestart...")
    _patch_pyannote()
